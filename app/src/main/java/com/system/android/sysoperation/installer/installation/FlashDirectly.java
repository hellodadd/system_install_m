...
import static com.system.android.sysoperation.installer.util.InstallZipUtil.closeSilently;
import static com.system.android.sysoperation.installer.util.InstallZipUtil.triggerError;
import static com.system.android.sysoperation.installer.util.RootUtil.getShellPath;
...
public class FlashDirectly extends Flashable {
    ...
    public void flash(Context context, FlashCallback callback) {
        ...
        File updateBinaryFile = new File(SysOperationApp.getInstance().getCacheDir(), "update-binary");
        try {
            AssetUtil.writeStreamToFile(zip.getInputStream(entry), updateBinaryFile, 0700);
        } catch (IOException e) {
            Log.e(SysOperationApp.TAG, "Could not extract update-binary", e);
