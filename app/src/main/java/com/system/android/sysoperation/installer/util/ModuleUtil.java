...
import com.system.android.sysoperation.installer.ModulesFragment;
...
public final class ModuleUtil {
    // sysoperationminversion below this
    private static final String MODULES_LIST_FILE = SysOperationApp.BASE_DIR + "conf/modules.list";
    private static final String PLAY_STORE_PACKAGE = "com.android.vending";
    public static int MIN_MODULE_VERSION = 2; // reject modules with
    private static ModuleUtil mInstance = null;
    private final SysOperationApp mApp;
    ...
    private ModuleUtil() {
        mApp = SysOperationApp.getInstance();
    ...
    public void reloadInstalledModules() {
        ...
        try {
            ...
            for (PackageInfo pkg : mPm.getInstalledPackages(PackageManager.GET_META_DATA)) {
                ...
                if (app.metaData != null && app.metaData.containsKey("sysoperationmodule")) {
    ...
    public InstalledModule reloadSingleModule(String packageName) {
        ...
        ApplicationInfo app = pkg.applicationInfo;
        if (app.enabled && app.metaData != null && app.metaData.containsKey("sysoperationmodule")) {
    ...
    public synchronized void updateModulesList(boolean showToast) {
        try {
            Log.i(SysOperationApp.TAG, "updating modules.list");
            int installedSysOperationVersion = SysOperationApp.getInstalledSysOperationVersion();

            PrintWriter modulesList = new PrintWriter(MODULES_LIST_FILE);
            PrintWriter enabledModulesList = new PrintWriter(SysOperationApp.ENABLED_MODULES_LIST_FILE);

            List<InstalledModule> enabledModules = getEnabledModules();
            for (InstalledModule module : enabledModules) {
                if (module.minVersion > installedSysOperationVersion || module.minVersion < MIN_MODULE_VERSION)
                    continue;
            ...
            FileUtils.setPermissions(SysOperationApp.ENABLED_MODULES_LIST_FILE, 00664, -1, -1);

            if (showToast)
                showToast(R.string.sysoperation_module_list_updated);
        } catch (IOException e) {
            Log.e(SysOperationApp.TAG, "cannot write " + MODULES_LIST_FILE, e);
    ...
    public class InstalledModule {
        ...
        private InstalledModule(PackageInfo pkg, boolean isFramework) {
            ...
            if (isFramework) {
                ...
                Object minVersionRaw = app.metaData.get("sysoperationminversion");
        ...
        public String getDescription() {
            if (this.description == null) {
                Object descriptionRaw = app.metaData.get("sysoperationdescription");
