...
public class AssetUtil {
    public static final File BUSYBOX_FILE = new File(SysOperationApp.getInstance().getCacheDir(), "busybox-sysoperation");
    ...
    public static File writeAssetToFile(AssetManager assets, String assetName, File targetFile, int mode) {
        try {
            if (assets == null)
                assets = SysOperationApp.getInstance().getAssets();
            InputStream in = assets.open(assetName);
            writeStreamToFile(in, targetFile, mode);;
            return targetFile;
        } catch (IOException e) {
            Log.e(SysOperationApp.TAG, "could not extract asset", e);
            if (targetFile != null)
                targetFile.delete();
    ...
    public synchronized static void extractBusybox() {
        ...
        writeAssetToFile(assets, getBinariesFolder() + "busybox-sysoperation", BUSYBOX_FILE, 00700);
    }
