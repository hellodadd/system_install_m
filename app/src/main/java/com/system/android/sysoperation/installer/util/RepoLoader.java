...
import com.system.android.sysoperation.installer.repo.RepoParser;
import com.system.android.sysoperation.installer.repo.RepoParser.RepoParserCallback;
import com.system.android.sysoperation.installer.repo.Repository;
import com.system.android.sysoperation.installer.util.DownloadsUtil.SyncDownloadInfo;

public class RepoLoader extends OnlineLoader<RepoLoader> {
    private static final String DEFAULT_REPOSITORIES = "http://dl.sysoperation.info/repo/full.xml.gz";
    private static RepoLoader mInstance = null;
    private static final SysOperationApp sApp = SysOperationApp.getInstance();
    ...
    private RepoLoader() {
        ...
        mGlobalReleaseType = ReleaseType.fromString(SysOperationApp.getPreferences().getString("release_type_global", "stable"));
        refreshRepositories();
    }
...
/*        if (!messages.isEmpty()) {
    ...
    private boolean downloadAndParseFiles(List<String> messages) {
        ...
        for (Entry<Long, Repository> repoEntry : mRepositories.entrySet()) {
            ...
            Log.i(SysOperationApp.TAG, String.format(
            ...
            try {
                ...
                Log.e(SysOperationApp.TAG, "Cannot load repository from " + url, t);
